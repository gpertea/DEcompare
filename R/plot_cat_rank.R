...
#' @export
plot_cat_rank <- function(filtered_dt1, filtered_dt2) {
  p <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "CAT-rank plot\n(Not implemented yet)", size = 5, hjust = 0.5) +
    theme_void() +
    labs(title = "CAT-rank Analysis")
  return(p)
}